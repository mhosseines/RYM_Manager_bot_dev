...
load_dotenv()
MIRROR_BALE_USERNAME = (os.getenv("BALE_CHANNEL_USERNAME") or "").lstrip("@").casefold()
...
async def fetch_all_channels(notify_callback):
    ...
    async with aiohttp.ClientSession() as session:
        for source in sources:
            ...
            # The configured mirror channel is a separate workflow: never queue it for review.
            if MIRROR_BALE_USERNAME and username.casefold() == MIRROR_BALE_USERNAME:
                logger.warning("[%s] skipped: this channel is configured for direct Bale → Telegram sync.", source_name)
                continue
